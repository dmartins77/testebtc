...
async function generateKeys() {
    ...
    while (continuarLoop) {
        ...
        if (Date.now() - startTime > segundos){
            segundos += 1000
            if (segundos % 600000 == 0){     //Exibe estatística a cada 10 minutos.
                const tempo = (Date.now() - startTime) / 1000;
                //console.clear();
                console.log('Resumo: ')
                console.log('Velocidade:', Math.round((Number(key) - Number(min))/ tempo), ' chaves por segundo')
